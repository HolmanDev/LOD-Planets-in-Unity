...
{
    public static int quadRes = 12; // The resolution of the quads (24)
    ...
    private void Awake()
    {
        if(!Generated) GenerateQuadTemplate(quadRes);
    }
    ...
    public void GenerateQuadTemplate(int res)
    {
        ...
        for (int quadI = 0; quadI < 16; quadI++) {
            ...
            for (int y = 0; y < (res + 1); y++)
            {
                ...
                {
                    Vector3 pos = new Vector3(x - res * 0.5f, y - res * 0.5f, 0) / (quadRes / 2);
    ...
    public void ClearQuadTemplate() {
        for (int quadI = 0; quadI < 16; quadI++) {
            quadTemplateVertices[quadI] = null;
            quadTemplateBorderVertices[quadI] = null;
            quadTemplateTriangles[quadI] = null;
            quadTemplateBorderTriangles[quadI] = null;
            quadTemplateEdgeIndices[quadI] = null;
        }
